...
public class Serv : IService
{
    ...
    public DataSet eTransactionCount_Registration(string QryDate, string UserID, string Password)
    {
        ...
        if (dt > 0)
        {
            ...
            ServiceSqlStr = @"select 'A098189006042' as 'ServiceCode',COUNT(*) as 'ServiceCount',convert(varchar,rdate,103) t_date from registration 
    ...
    public DataSet eTransactionCount_Application(string QryDate, string UserID, string Password)
    {
        ...
        if (dt > 0)
        {
            ...
            //}
            //ServiceSqlStr = "select SubServiceCode as 'ServiceCode',NoofApplications as 'ServiceCount' from cenmon.centralEPPService where convert(varchar,PDate,112)=@QryDate";
            ServiceSqlStr = @"select 'A098191806044' as 'ServiceCode',COUNT(*) as 'ServiceCount',convert(varchar,receive_dt,103) t_date from JobApplication 
    ...
    public DataSet eTransactionCount_Fee(string QryDate, string UserID, string Password)
    {
        ...
        if (dt > 0)
        {
            ...
            //}
            //ServiceSqlStr = "select SubServiceCode as 'ServiceCode',NoofApplications as 'ServiceCount' from cenmon.centralEPPService where convert(varchar,PDate,112)=@QryDate";
            ServiceSqlStr = @"select 'A098189006045' as 'ServiceCode',COUNT(*) as 'ServiceCount',convert(varchar,trandate,103) t_date from FeeDetails 
    ...
    public Int32 ValidateUser(string userName, string password)
    {
        ...
        if (usr.Rows.Count > 0)
        {
            ...
            password = FormsAuthentication.HashPasswordForStoringInConfigFile(password, "md5").ToString().ToLower();
